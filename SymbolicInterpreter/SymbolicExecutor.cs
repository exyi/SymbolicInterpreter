...
namespace SymbolicInterpreter
{
    public class SymbolicExecutor
    {
        ...
        private static DisassemblyResult TransformDisassemblyResults<T>(Type returnType, Sigil.DisassembledOperations<T> op)
        {
            ...
            {
                IsCatchBlockEnd = s.IsCatchBlockEnd,
                IsCatchBlockStart = s.IsCatchBlockStart,
                IsExceptionBlockEnd = s.IsExceptionBlockEnd,
                IsExceptionBlockStart = s.IsExceptionBlockStart,
                IsFinallyBlockEnd = s.IsFinallyBlockEnd,
                IsFinallyBlockStart = s.IsFinallyBlockStart,
                IsFaultBlockStart = s.IsFaultBlockStart,
                IsFaultBlockEnd = s.IsFaultBlockEnd,
                IsMarkLabel = s.IsMarkLabel,
                IsOpCode = s.IsOpCode,
            ...
            return new DisassemblyResult
            {
                ...
                Labels = op.Labels.ToArray(),
                Locals = op.Locals.ToArray(),
                Parameters = op.Parameters.ToArray(),
        ...
        protected MethodContext CreateMethodContext(DisassemblyResult disassembly)
        {
            return new MethodContext
            {
                Disassembly = disassembly,
                Locals = disassembly.Locals.Select(l => MyExpression.Parameter(l.LocalType, $"local_{l.Name}__{Interlocked.Increment(ref pcnt)}")).ToArray(),
                Parameters = disassembly.Parameters.Select(p => MyExpression.Parameter(p.ParameterType, $"param_{p.Name}__{Interlocked.Increment(ref pcnt)}")).ToArray()
            };
        }
        ...
        private ExecutionState ExecCore(MethodContext mc, ExecutionState state, int eip)
        {
    ...
    {
        public DisassemblyResult Disassembly { get; set; }
        public MyParameterExpression[] Locals { get; set; }
        public MyParameterExpression[] Parameters { get; set; }
    ...
    {
        public MethodInstruction[] Instructions { get; set; }
        public Sigil.Label[] Labels { get; set; }
        public Sigil.Local[] Locals { get; set; }
        public Sigil.Parameter[] Parameters { get; set; }
